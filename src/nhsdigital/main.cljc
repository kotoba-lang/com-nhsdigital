(ns nhsdigital.main
  "Kotodama WASM entrypoint for the nhsdigital clean-room actor (L4) — Clojure port.
  ...
  facts (`nhsdigital.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nhsdigital")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "nhsdigit_fil"
    :fields [:filerId :scheme :period :status]   :required [:scheme :period]
    :coerce {}                       :refs {}}
   {:entity "Citizen"     :plural "citizens"     :id-prefix "nhsdigit_cit"
    :fields [:nationalId :jurisdiction]          :required [:jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Service"     :plural "services"     :id-prefix "nhsdigit_ser"
    :fields [:name :department]                  :required [:name :department]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "nhsdigit_app"
    :fields [:citizenId :serviceId :status]      :required [:status]
    :coerce {}                       :refs {:citizenId "Citizen" :serviceId "Service"}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "nhsdigit_dat"
    :fields [:name :publisher :updatedAt]        :required [:name :publisher]
    :coerce {}                       :refs {}}
   {:entity "Identity"    :plural "identities"   :id-prefix "nhsdigit_ide"
...
(defn emit-facts
  "EAVT facts for one record: {\"nhsdigital.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nhsdigital-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nhsdigital-compat/ready)
